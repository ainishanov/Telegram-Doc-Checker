...
`;
    ...
    if (!hasStructuredText) {
      ...
      const message = `📄 *Документ с текстом в виде изображений*\n\n` +
        `⚠️ *Внимание:* Документ содержит текст преимущественно в виде изображений, что делает невозможным его полноценный анализ.\n\n` +
        `*Рекомендации:*\n` +
        ...
        `   - onlineocr.net/ru\n\n` +
        `Выберите любую сторону для получения подробных рекомендаций:`;
...
function checkTextStructure(text) {
  ...
  const hasEnoughSentences = sentences.length >= 5;
  
  console.log(`Структура пунктов: ${hasParagraphStructure}, Достаточно предложений: ${hasEnoughSentences}`);
  
  // Общая проверка количества найденных ключевых слов и структуры
  if (keywordsFound >= 3 && (hasParagraphStructure || hasEnoughSentences)) {
    return true;
  }
  ...
  const hasOrganizations = orgMatches.length >= 2;
  
  console.log(`Найдено организаций/ИП: ${orgMatches.length}`);
  
  return keywordsFound >= 4 || (keywordsFound >= 2 && hasOrganizations);
}
