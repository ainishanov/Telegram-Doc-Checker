...
Документ определен как договор, но вы использовали все доступные запросы на тарифе "${PLANS.FREE.name}" (${PLANS.FREE.requestLimit} проверок).

Чтобы продолжить использование бота, выберите один из платных тарифов:

*${basicPlan.name}* - ${basicPlan.price} руб/мес
• ${basicPlan.requestLimit} проверок договоров в месяц
• ${basicPlan.description}

Для выбора тарифа используйте команду /plans
`;
...
Вы использовали все доступные запросы на тарифе "${PLANS.FREE.name}" (${PLANS.FREE.requestLimit} проверок).
...
`;
    ...
    if (isFirstFreeCheck) {
      ...
      limitInfo = '\n\n⚠️ Это была ваша последняя проверка на бесплатном тарифе. Для продолжения использования бота выберите платный тариф с помощью команды /plans.';
    } else if (updatedLimits.remainingRequests <= 2 && PLANS.FREE.requestLimit <= 3) {
      limitInfo = `\n\n⚠️ У вас осталось ${updatedLimits.remainingRequests} проверок на бесплатном тарифе.`;
    }
