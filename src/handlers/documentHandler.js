...
`;
    ...
    const message = analysis.incomplete 
      ? `📄 *Частичный анализ договора*\n\n` +
        `⚠️ *Внимание:* Документ содержит текст в виде изображений, анализ может быть неполным.\n\n` +
        `*Определены стороны договора:*\n` +
        `1️⃣ ${analysis.party1.role}: *${analysis.party1.name}*\n` +
        `2️⃣ ${analysis.party2.role}: *${analysis.party2.name}*\n\n` +
        `Для более точного анализа рекомендуется предоставить документ в текстовом формате или использовать OCR-сервис для преобразования изображений в текст.\n\n` +
