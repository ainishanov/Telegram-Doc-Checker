...
async function handleShowTariff(bot, msg) {
  ...
  if (planInfo.requestLimit === Infinity) {
    ...
    message += `📊 *Использовано договоров:* ${planInfo.requestsUsed} из ${planInfo.requestLimit}\n`;
    message += `📈 *Осталось договоров:* ${planInfo.requestsRemaining}\n`;
  }
...
async function handleShowPlans(bot, msg) {
  ...
  for (const plan of plans) {
    ...
    // Информация о количестве договоров (без упоминания проверок)
    if (plan.requestLimit === Infinity) {
      message += '• Безлимитное количество договоров\n';
    } else {
      if (plan.id === 'FREE') {
        message += `• ${plan.requestLimit} договора бесплатно навсегда\n`;
      } else {
        message += `• ${plan.requestLimit} договоров в месяц\n`;
      }
    }
    
    // Добавляем первые 2 особенности тарифа
    for (let i = 0; i < Math.min(2, plan.features.length); i++) {
      message += `• ${plan.features[i]}\n`;
    }
...
async function handleShowPlanDetails(bot, planId, msg) {
  ...
  if (plan.requestLimit === Infinity) {
    ...
    message += `📊 Количество договоров: *${plan.requestLimit} в месяц*\n\n`;
  }
...
async function handleSelectPlan(bot, planId, msg) {
  ...
  if (planId === 'FREE') {
    ...
    message += `У вас есть ${result.plan.requestLimit} договора бесплатно.\n`;
